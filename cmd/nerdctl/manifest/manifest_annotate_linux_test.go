...
func TestManifestAnnotate(t *testing.T) {
	...
	testCase.SubTests = []*test.Case{
		...
		{
			Description: "annotate-with-unsupported-os-arch",
			Setup: func(data test.Data, helpers test.Helpers) {
				cmd := helpers.Command("manifest", "create", manifestListName+"-unsupported", manifestRef)
				cmd.Run(&test.Expected{ExitCode: 0})
			},
			Command: test.Command("manifest", "annotate",
				manifestListName+"-unsupported",
				manifestRef,
				"--os", "freebsd",
				"--arch", "fake",
				"--os-version", "1",
			...
			Data: test.WithLabels(map[string]string{
				"error": "manifest entry for image has unsupported os/arch combination",
			}),
		},
		{
			Description: "annotate-with-valid-os-arch",
			Setup: func(data test.Data, helpers test.Helpers) {
				cmd := helpers.Command("manifest", "create", manifestListName+"-valid", manifestRef)
				cmd.Run(&test.Expected{ExitCode: 0})
			},
			Command: test.Command("manifest", "annotate",
				manifestListName+"-valid",
				manifestRef,
				"--os", "freebsd",
				"--arch", "arm",
				"--os-version", "1",
