...
func PullImageWithTransfer(ctx context.Context, client *containerd.Client, scheme, ref, ipfsPath string, options types.ImagePullOptions) (*imgutil.EnsuredImage, error) {
	...
	// Use local transfer service directly to avoid serialization issues.
	// The proxy transfer service (client.Transfer) requires types to be registered
	// with typeurl, but IPFSSource is not registered. For local operations,
	// we can use the local transfer service directly without serialization.
	transferService := newLocalTransferService(client)
	if err := transferService.Transfer(ctx, source, dest, transferOpts...); err != nil {
...
func PushImageWithTransfer(ctx context.Context, client *containerd.Client, rawRef, ipfsPath string, options types.ImagePushOptions) (string, error) {
	...
	// Use local transfer service directly to avoid serialization issues.
	// The proxy transfer service (client.Transfer) requires types to be registered
	// with typeurl, but IPFSDestination is not registered. For local operations,
	// we can use the local transfer service directly without serialization.
	transferService := newLocalTransferService(client)
	if err := transferService.Transfer(ctx, source, dest, transferOpts...); err != nil {
...
// newLocalTransferService creates a local transfer service for IPFS operations.
// This is used instead of client.Transfer() to avoid type serialization requirements.
func newLocalTransferService(client *containerd.Client) transfer.Transferrer {
	return transferlocal.NewTransferService(
		client.ContentStore(),
		client.ImageService(),
		transferlocal.TransferConfig{
			// Use default configuration for IPFS transfers
		},
	)
}
